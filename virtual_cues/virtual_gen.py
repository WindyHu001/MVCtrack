...
class PaintDataSet(Dataset):
    ...
    def __getitem__(self, index):
        ...
        for path in all_cams_path:
            path = path.replace('/data/v1.0/trainval', 'data/nuScenes')
...
@torch.no_grad()
def add_virtual_mask(masks, labels, points, raw_points, num_virtual=120, dist_thresh=3000, num_camera=6, intrinsics=None, transforms=None):
    ...
    lidar_position = torch.tensor([0, 0, 0], device=points.device).float()  # 假设激光雷达位于(0, 0, 0)
    #
    lidar_position = lidar_position.unsqueeze(0).unsqueeze(0)
    distances = torch.norm(points[:, :, :3] - lidar_position, dim=2).reshape(-1).unsqueeze(1)
    result = valid * distances

    object_distance_list = result.sum(dim=0) / valid.sum(dim=0)
    ...
    for i,mask in enumerate(masks):
        ...
        if not torch.isnan(object_distance_list[i]):
            object_distance = object_distance_list[i]
            #import pdb;pdb.set_trace()
            ratio = torch.tensor(object_distance / avg_distance, device=points.device)
            scaling_factor = max(1.0, max(torch.sigmoid(ratio) + 1.0,2))
            dynamic_num_virtual = int(min(num_virtual / (3 - scaling_factor), 60 * 2))
            num_virtual = dynamic_num_virtual
            torch.cuda.empty_cache()
        ...
        if len(selected_indices) < 120:
                ...
                selected_indices = torch.cat([selected_indices, selected_indices[
                    selected_indices.new_zeros(120-len(selected_indices))]])
...
def main(args):
    ...
    for idx, data in tqdm(enumerate(data_loader), total=len(data_loader.dataset)):
        ...
        output_path = output_path.replace('data/v1.0/trainval', 'data/nuScenes')
...
if __name__ == '__main__':
    ...
    if not os.path.isdir('data/nuScenes/samples/LIDAR_TOP_VIRTUAL_3'):
        os.mkdir('data/nuScenes/samples/LIDAR_TOP_VIRTUAL_3')

    if not os.path.isdir('data/nuScenes/sweeps/LIDAR_TOP_VIRTUAL_3'):
        os.mkdir('data/nuScenes/sweeps/LIDAR_TOP_VIRTUAL_3')
